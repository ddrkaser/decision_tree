...
preprocessor = ColumnTransformer([('one-hot-encoder', categorical_preprocessor, categorical_columns)],remainder='passthrough',verbose_feature_names_out=False)
...
plot_tree(model['decisiontreeclassifier'],
          filled=True,
          rounded=True,
...
ConfusionMatrixDisplay.from_estimator(model, X_test, y_test, display_labels=["Does not have HD", "Has HD"])


path = model['decisiontreeclassifier'].cost_complexity_pruning_path(X_train, y_train) # determine values for alpha
...
for ccp_alpha in ccp_alphas:
    clf_dt = make_pipeline(preprocessor, DecisionTreeClassifier(random_state=42,ccp_alpha=ccp_alpha))
...
#cross validation
clf_dt = make_pipeline(preprocessor, DecisionTreeClassifier(random_state=42, ccp_alpha=0.016)) # create the tree with ccp_alpha=0.016
...
clf_dt_pruned = make_pipeline(preprocessor, DecisionTreeClassifier(random_state=42, ccp_alpha=ideal_ccp_alpha))
...
plt.figure(figsize=(60, 30))
plot_tree(clf_dt_pruned['decisiontreeclassifier'],
          filled=True,
          rounded=True,
          class_names=["No HD", "Yes HD"],
          feature_names=feature_names);
